...
describe('ActivityFeed', () => {
  it('renders each activity item with its description', () => {
    const activity: ActivityItem[] = [
      ...
      { type: 'MEMBER_JOINED', referenceId: 'm-1', description: 'Maria Silva (Aluno) ingressou na organização', occurredAt: '2026-01-12T10:00:00' },
    ]

    render(<ActivityFeed activity={activity} />)

    expect(screen.getByText('Turma "Matemática" criada')).toBeTruthy()
    expect(screen.getByText('Maria Silva (Aluno) ingressou na organização')).toBeTruthy()
  })
