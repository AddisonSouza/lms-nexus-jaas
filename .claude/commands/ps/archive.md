...
6b. **GitHub Integration — PR + close issue + move to "Archived" (optional)**

   Read `pscode/github.yaml` with the **Read tool**. If not found, skip this step.

   **6b-i. Detect branch** — check if a branch `feature/*-<change-name>` or `feature/<change-name>` exists:
   ```bash
   git branch --list "feature/*<change-name>*"
   ```

   **6b-ii. Push branch if found:**
   ```bash
   git push -u origin "<branch-name>"
   ```

   **6b-iii. Create Pull Request** using the Bash tool with the `gh` path from `github.yaml`:
   ```bash
   "<gh>" pr create \
     --repo "<repo>" \
     --head "<branch-name>" \
   ...
   - Implementa **<RF-NN> — <title>** conforme especificado em `pscode/changes/archive/<archived-name>/`
   - Artefatos: proposal.md · specs · design.md · tasks.md

   ## Spec

   Capability: `pscode/specs/<capability>/spec.md`

   ## Checklist

   - [ ] Migrations Flyway incluídas
   - [ ] Testes unitários e de integração
   - [ ] Sem `any` ou `@ts-ignore` no frontend
   - [ ] OpenAPI documentado no endpoint
   ...
   Save the returned PR URL as `prUrl`.

   **6b-iv. Sync issue status** — invoke **Skill tool** with `pscode-gh-sync`:
   - `action`: `archive`
   - `change`: `<name>`
   - `summary`: PR URL (e.g. "PR criado: <prUrl>")

   Fire-and-forget on both the PR creation and the sync — never block if they fail.
