...
2b. **GitHub Integration — create branch + move issue to "In Progress" (optional)**

   Read `pscode/github.yaml` with the **Read tool**. If not found, skip this step.

   **2b-i. Detect issue number** from the change name (`rf-(\d+)` → issue #N) or from `links` map in github.yaml.

   **2b-ii. Fetch module label** — if issue number found, run:
   ```bash
   "<gh>" issue view <N> -R "<repo>" --json labels
   ```
   Extract the label matching `module: *` (e.g. `module: identity` → `identity`). If not found, derive the module from the change name or use the change name as-is.

   **2b-iii. Create and checkout a feature branch** named `feature/<module>-<change-name>`:

   ```bash
   git checkout -b "feature/<module>-<change-name>"
   ```

   If the branch already exists, check it out instead:
   ```bash
   git checkout "feature/<module>-<change-name>"
   ```

   **2b-iii. Sync issue status** — invoke the **Skill tool** with `pscode-gh-sync`:
   - `action`: `apply`
   - `change`: `<name>`

   Fire-and-forget on the sync — never block if it fails. Branch creation is also best-effort.

3. **Load architecture context from repo docs**

   Use the **Read tool** for all reads — do NOT use shell commands.

   a. Read `docs/architecture/DECISIONS.md`. Store as `architectureContext`.
   b. Detect RF number from change name (`rf-(\d+)`). If found, read the matching section of `docs/requirements/RF.md`. Store as `rfContext`.
   c. Read the `design.md` artifact from the change directory (available in `artifactPaths` after step 4). Store as `designContext`.

   Apply `architectureContext` and `rfContext` as hard constraints throughout implementation:
   - Package structure, naming conventions, Lombok rules (LOM-01 to LOM-07)
   - Layer dependency rules (RD-01 to RD-08)
   - Security rules (SEC-01 to SEC-08), persistence rules (DB-01 to DB-08)
   - Frontend rules (FE-01 to FE-12)
